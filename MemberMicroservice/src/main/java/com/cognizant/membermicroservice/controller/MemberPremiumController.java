...
public class MemberPremiumController {
   ...
   public List<MemberPremium> billDetails(@PathVariable("policyid")int policyid)
   {
	...
	public ClaimStatusDTO submitClaim(@PathVariable int policyId, @PathVariable int claimId,
		...
		ClaimStatusDTO claimResponse = claimsClient.submitClaim(policyId,claimId, memberId, providerId,
				benefitId, totalAmount, claimedAmount);
		
		return claimResponse;
	}*/
	
	@PostMapping("/submitClaim/{policyId}/{claimId}/{memberId}/{providerId}/{benefitId}/{totalAmount}/{claimedAmount}")
	public ResponseEntity<MemberClaim> submitClaim(@PathVariable int policyId, @PathVariable int claimId,
			@PathVariable int memberId, @PathVariable int providerId, @PathVariable int benefitId,
			@PathVariable double totalAmount, @PathVariable double claimedAmount) {
		...
		uriVariables.put("totalAmount", (int) totalAmount);
		uriVariables.put("claimedAmount", (int) claimedAmount);
		
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON);
		
		String url = "http://localhost:8090/claims/submitClaim/{policyId}/{claimId}/{memberId}/{providerId}/{benefitId}/{totalAmount}/{claimedAmount}";
		MemberClaim memclaimobj = new MemberClaim();
		
		HttpEntity<MemberClaim> requestEntity = new HttpEntity<>(memclaimobj, headers);
		ResponseEntity<MemberClaim> postForEntity = rt.postForEntity(url,requestEntity,MemberClaim.class, uriVariables);
